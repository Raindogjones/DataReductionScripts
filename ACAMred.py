...
#	Correct limits for ACAM to trim most of none-exposed region.  DOES cut away some data region as the transmission is circular, but this is important or photometry script alignment (fails with too much zero level background).
        image_list[idx] = ccdproc.trim_image(img, fits_section='[100:1900,250:2050]', add_keyword={'trimmed': True, 'calstat': 'OT'})
...
for flat, fname in images.hdus(IMAGETYP='sky', return_fname=True):
	meta = flat.header
	if meta["ACAMFILT"].replace("CLEAR","").replace("+","") not in flatbands:
		flatbands.append(meta["ACAMFILT"].replace("CLEAR","").replace("+",""))
...
for band in flatbands:
	...
	for flat, fname in images.hdus(IMAGETYP='sky', return_fname=True):
		meta=flat.header
		if meta["ACAMFILT"].replace("CLEAR","").replace("+","")==band:
...
#     meta = flat.header
#     if meta["ACAMFILT"] not in flatbands:
...
#     	meta = star.header
#     	meta['filename'] = fname
#     	if meta["ACAMFILT"]==flat.header["ACAMFILT"]:
