...
arcsec_rad=2.2
selectnewcomp='y' #Change to anything but 'y' in order to use previous comparison selections
...
if selectnewcomp=='y':
	...
	os.system('/star-2018A/bin/gaia/gaia.sh ref.fit')
...
#	starflux,stardflux,flag=sep.sum_circle(data-bkg.back(),x-1,y-1,aper,err=bkg.globalrms,gain=ganancia) #background subtraction
	starflux,stardflux,flag=sep.sum_circle(data,x-1,y-1,aper,bkgann=(aper*1.5,aper*2),err=bkg.globalrms,gain=ganancia) #sky annulus
#	starflux,stardflux,flag=sep.sum_circle(data-bkg.back(),x-1,y-1,aper,bkgann=(aper*1.5,aper*2),err=bkg.globalrms,gain=ganancia)
...
# 	hjd=tmid.jd+tdelta.jd
	...
	print file, hjd.jd, starflux[0], magtar, magc1, tarc1err
...
#	print file,magcsub,errmagc
	...
	if tarc1err < 0.2 and not tarc1=="nan" :
		print >>differential, "%.8f %.5f %.5f %.5f %.5f" %(hjd.jd, tarc1, tarc1err, magcsub, errmagc)
