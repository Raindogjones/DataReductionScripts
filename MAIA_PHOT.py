...
if images_to_align:
	identifications = alipy.ident.run(ref_image, images_to_align, visu=False, hdu=hdunum)
	...
	outputshape = alipy.align.shape(ref_image, hdu=hdunum)

	for id in identifications:
		if id.ok == True:
			alipy.align.irafalign(id.ukn.filepath, id.uknmatchstars, id.refmatchstars, shape=outputshape, makepng=False, hdu=hdunum)
			
	for image in images_to_align:
		imagereg="alipy_out/"+image[:-5]+"_gregister.fits"
		if exists (imagereg):
			hdu=fits.open(image)
			header=hdu[0].header
			RA=header['RA']
			DEC=header['DEC']
			hdureg=fits.open(imagereg,mode='update')
			hdureg[0].header['RA']=RA
			hdureg[0].header['DEC']=DEC
			hdureg.flush()
...
#	starflux,stardflux,flag=sep.sum_circle(data-bkg.back(),xpos-1,ypos-1,aper,err=bkg.globalrms,gain=ganancia,bkgann=(aper+1, aper+2))
	...
	try:
		...
		try:
			...
			print "Couldn't find header keyword for date.  Skipping...\n"
			continue
