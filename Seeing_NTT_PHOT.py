...
#The aperture radius as a function of seeing
seeing_rad=1.5
...
images2=[w.replace('_gregister.fits','.fits') for w in images2]
...
#commands to get seeing
	...
	ycen=np.zeros(3)
	for x in range(0,3):
		monkey=test.gauss_center(xpos[x],ypos[x],data,delta=10)
...
#	print file, starflux
	...
	if starflux[0]==0:
		print "Negative aperture sum on %s" %file
		continue
	elif starflux[1]==0:
		print "Negative aperture sum on %s" %file
		continue
...
#	print file,magcsub,errmagc
	...
	if not tarc1=="nan" :
		...
		airmass=header['HIERARCH ESO TEL AIRM START']
